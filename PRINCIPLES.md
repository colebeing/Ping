...
- Cadence collapsing from three parallel "frequency" modes (once/twice/four, each with its own block ids and its own admin content surface) to one always-four-blocks-with-per-block-skip model ([today.ts](frontend/src/views/today.ts)'s `blocksForCadence`) is a deliberate trade named explicitly rather than picked by default: a 3-option dropdown is fewer controls than 4 independent skip toggles in Settings, but it collapses three schemes worth of branching into one model that's easier to reason about and extend — net simplification of the system, at the cost of slightly more surface in one screen. Worth naming as the kind of trade-off this principle asks to be explicit about, not just quietly resolved.

**Where it holds today (cont'd):**
- The four categories were renamed from `friends`/`colleagues`/`family`/`me` to `environment`/`people`/`impact`/`capacity` — the EPIC set, always presented in exactly that order ([types.ts](backend/src/types.ts)'s `CATEGORIES`) — a deliberate named framework this time, not four words that happened to seem reasonable. Existing KV data (every user's answer history, the live question tree, Sheet sync) upgrades transparently on read rather than resetting anyone's history — see [category-migration.ts](backend/src/category-migration.ts).

**Where it's in tension:**
- A reason existing and a reason being *written down* are different things, and this principle only asks for the first — that's really Hypervisible's job (below). Concretely: `BLOCK_SWITCH_DELAY_MINUTES` is explained as "why a delay," but not "why 60 minutes and not 30 or 90." The EPIC rename gives the category set a name and a fixed order, but not yet a citable reason for those specific four dimensions over some other four — that deeper "why EPIC" question is still open, and still Hypervisible's gap below, not resolved by naming alone.
...
- `detectStreaks` treats a run of "no" answers as an invitation to help ("resolve" — offer to re-center the check-in on that category), not as a flag to escalate or scold ([recommendations.ts](backend/src/recommendations.ts)).
- Retirement never counts a "no" against a question. Decided 2026-09-24, replacing a rule where any single "no" after accepting a swap permanently blocked the question from retiring, which quietly made "no" costly. Now a swapped-in question retires only when it goes completely unanswered: no answers in any block for `returnAfterUnansweredDays` (3) full days in a row. Even then nothing changes silently. Its parent's swap invite, or "Want to go back to your usual check-in?" when the parent is the routine question, is asked in place of the question, and the user decides. Declining keeps the question and restarts the count. Answering at all cancels a pending step-back invite. See `checkUnanswered` in [recommendations.ts](backend/src/recommendations.ts).
