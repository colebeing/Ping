...
- Onboarding itself is now one tap: no email, no password, not even a screen to read past — granting notification permission is the entire signup ([onboarding.ts](frontend/src/views/onboarding.ts), `api.startAnonymous()`). An email/Google account moved from "required before you can start" to "optional, added later from Settings."
...
- Denying notification permission at onboarding still lets you in — Ping's whole loop runs on the nudge, so a persistent (non-blocking) reminder banner sits on Home every time it's off, reappearing on every visit rather than just once. This is a functional prompt about a permission the product needs to work, not a manipulative one — it never references streaks, time invested, or what you'll lose, and it never blocks the actual check-in card underneath it.
- Suggesting a real email/Google account, by contrast, is capped at exactly **one** appearance ever — a single quiet banner after a few days of anonymous use ([home.ts](frontend/src/views/home.ts)'s `ACCOUNT_NUDGE_AFTER_DAYS`), tracked via `localStorage` so it truly never repeats. Framed only as "you can save this," never "you'll lose this" — outside that one moment, it's a permanently-available, easy-to-ignore option in Settings, never a proactive nag. The difference between this and the notification banner is deliberate: one is about the product working at all (repeat forever), the other is about data safety (say it once, respectfully, then stop).
